# 👋 Hi, I'm Anitha Amaravadi

### 🎓 B.Tech Artificial Intelligence & Machine Learning Student

I'm a passionate developer student interested in building real-world projects
and continuously improving my programming and development skills.

## 💻 Skills

- ☕ Java
- 🐍 Python
- 🌐 HTML, CSS, JavaScript
- 🗄️ SQL
- 🤖 Artificial Intelligence & Machine Learning
- 📊 Data Structures & Algorithms

## 🚀 Currently Learning

- Full Stack Web Development
- Java & DSA
- React
- Spring Boot
- AI & Machine Learning

## 📂 Projects

- 🌐 **Servixa** – Service-based web application
- 🌦️ **Weather Pulse** – Weather application
- 📱 **QR Code Generator** – Multi-purpose QR generator
- 🤖 **Face Recognition System** – AI-based project

## 🎯 Goal

To become a skilled Software Developer and build innovative
AI-powered applications that solve real-world problems.

## 📫 Connect With Me

- 💼 LinkedIn: [Your LinkedIn](https://www.linkedin.com/in/anitha-amaravadi3/)
   
---

⭐ Thanks for visiting my profile! 
    
